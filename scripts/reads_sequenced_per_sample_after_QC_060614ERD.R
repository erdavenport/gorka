...
lines <- read.table(file="/Users/erdavenport/clusterhome/gorka/data/summaries/fastq_lines_for_each_sample_after_QC_060614ERD.txt", sep=" ", header=FALSE, stringsAsFactors=FALSE)
...
# Save table:
write.table(sorted, paste("/Users/erdavenport/Dropbox/Lab/Gorka/results/table.reads.sequenced.per.sample.after.QC.",today,"ERD.txt", sep=""), sep="\t", row.names=FALSE, quote=FALSE)

pdf(paste("/Users/erdavenport/Dropbox/Lab/Gorka/results/barplot.reads.sequenced.per.sample.after.QC.",today,"ERD.pdf", sep=""), width=10, height=5)
par(mfrow=c(1,2))

# regular scale:
barplot(sorted$V1, col=sapply(sorted$V2, colme), main="Reads sequenced per sample\n(after QC)", ylab="reads", xlab="sample")
...
# log scale:
barplot(log10(sorted$V1), col=sapply(sorted$V2, colme), main="Reads sequenced per sample\n(after QC)", ylab="log10(reads)", xlab="sample")
